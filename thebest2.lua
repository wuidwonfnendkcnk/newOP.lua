...
Fire=function(pos)
  ...
  Ball.Velocity=Ball.CFrame.lookVector*500
  ...
  Ball.Touched:connect(function(h)
    ...
    for i=1,10 do
      ...
      block.CFrame=CFrame.new(Ball.Position)*CFrame.new(0,-8,0)
      ...
      rek.Touched:connect(function(h) if h.Name==b.Name then return end pcall(function() if h.Parent~=p.Character then h.Parent.Humanoid:TakeDamage(10) end end) end)
    end
...
mouse.Button1Down:connect(function()
  ...
  for i=1,End,ind do
    ...
    laz.Touched:connect(function(h) if h.Name==b.Name then return end  if h.Parent:findFirstChild'Humanoid' and h.Name~=p.Name then h.Parent.Humanoid:TakeDamage(40) end end)
    local bl=p:clone()
    bl.Parent=p
    bl.Name='ball'
    ...
    bl.Transparency=1
    bl.Touched:connect(function(h) if h.Name==b.Name then return end if h.Parent:findFirstChild'Humanoid' and h.Name~=p.Name then h.Parent.Humanoid:TakeDamage(40) end end)
    ...
    block.Material='Neon'
    block.Touched:connect(function(h) if h.Name==b.Name then return end if h.Parent:findFirstChild'Humanoid' and h.Name~=p.Name then h.Parent.Humanoid:TakeDamage(40) end end)
    bl.CanCollide=false
  end
   for i=End,1,-ind do
    wait()
...
if (Vector3.new(pp.X,0,pp.Z)-Vector3.new(PP.X,0,PP.Z)).magnitude >1 then
  Moving=true
else
  Moving=false
  end
